...
@add_start_docstrings(
    """Albert Model with a with a multiple choice classification head on top (a linear layer on top of
    the pooled output and a softmax) e.g. for RocStories/SWAG tasks. """,
    ALBERT_START_DOCSTRING,
    #ALBERT_INPUTS_DOCSTRING, # since multiple choice input dimensions are different from normal input.
)
class AlbertForMultipleChoice(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2')
        model = AlbertForMultipleChoice.from_pretrained('roberta-base-v2')
    ...
    def __init__(self, config):
        ...
        self.albert = AlbertModel(config)
        self.dropout = nn.Dropout(0.1) #nn.Dropout(config.hidden_dropout_prob) TODO: should we increase a new file?
